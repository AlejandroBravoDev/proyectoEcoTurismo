...
const SearchBar = ({
  ...
  const selectedMunicipioLabel = useMemo(() => {
    ...
    if (!currentMunicipioId || lista.length === 0)
      return "Todos los Municipios";

    const encontrado = lista.find(
      (m) => m.id.toString() === currentMunicipioId.toString(),
    );
    return encontrado ? encontrado.nombre : "Todos los Municipios";
  ...
  return (
    <div className="bg-white p-3 md:p-4 rounded-[30px] md:rounded-full shadow-2xl flex flex-col md:flex-row items-center gap-3 w-full max-w-5xl border border-gray-100 transition-all hover:shadow-[#20A217]/10 relative z-50">
      ...
      <div className="relative w-full md:w-72" ref={dropdownRef}>
        ...
        {isOpen && (
          <div className="absolute top-[115%] left-0 w-full bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-[60]">
            <div className="max-h-60 overflow-y-auto custom-scrollbar">
              <button
                ...
                className="w-full text-left px-6 py-3 hover:bg-green-50 text-gray-600 font-medium transition-colors border-b border-gray-50"
              >
                ...
                municipios.data.map((m) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-6 py-3 hover:bg-green-50 transition-colors flex items-center justify-between ${
                        isSelected
                          ? "bg-green-50 text-[#20A217] font-bold"
                          : "text-gray-600"
                      }`}
                    >
                      {m.nombre}
                      {isSelected && (
                        <div className="w-2 h-2 rounded-full bg-[#20A217]" />
                      )}
                    </button>
                  );
                })}
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #20A217; border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #1a8212; }
      `}</style>
    </div>
  );
};
